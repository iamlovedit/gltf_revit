...
本文面向维护导出器的工程师，说明 `AutoCadGltfExporter` 如何把 DWG 图元转换为 glTF 2.0 Binary 文件。核心代码位于 `src/AutoCadGltfExporter` 和 `src/Shared`。
...
共享模块 `src/Shared/GltfSchema.cs` 中的 `GltfRoot` 对应 glTF JSON 根对象。当前 AutoCAD 导出主要写入这些字段：
