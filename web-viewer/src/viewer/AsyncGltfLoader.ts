...
function getLoader(): GLTFLoader {
  ...
  draco.setWorkerLimit(Math.max(1, Math.min(4, navigator.hardwareConcurrency ?? 2)));
...
export async function loadGlb(url: string, opts: LoadOptions = {}): Promise<GLTF> {
  ...
  while (true) {
    ...
    if (value) {
      ...
      else if (opts.onProgress) opts.onProgress(Math.min(0.95, received / (received + 1_000_000)));
    }
  }
  opts.onProgress?.(1);
